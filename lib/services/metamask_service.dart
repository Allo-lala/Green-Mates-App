  ...
  Future<String?> connectWallet({BuildContext? context}) async {
    ...
    while (_connectionAttempts < _maxRetries) {
      try {
        ...
        final connectResponse = await _connector!.connect(
          optionalNamespaces: {
            'eip155': RequiredNamespace(
              chains: ['eip155:42220'], // Celo network
          ...
          onTimeout: () {
            throw ConnectionException('Connection timeout. Please try again.');
          },
        );
        ...
        _connectionAttempts++;
        AppLogger.e(
            'Connection attempt $_connectionAttempts failed: ${e.message}', e);
